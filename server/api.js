...
api.get('/funding', (req, res, next) => {
  db.all(
    `SELECT investment.*, 
    GROUP_CONCAT(funding.amount) AS amounts
    FROM investment 
    LEFT OUTER JOIN funding 
    ON funding.investment_id = investment.id
    WHERE fully_funded = 0
    GROUP BY funding.investment_id
    `,
    (err, rows) => {
      if (err) {
        next(err)
...
api.put('/funding/complete', (req, res, next) => {
  db.serialize(() => {
    db.run(
      `UPDATE investment SET fully_funded = 1 WHERE id = ?`,
      req.body.investment_id,
      (err, row) => {
        if (err) {
          next(err)
        }
